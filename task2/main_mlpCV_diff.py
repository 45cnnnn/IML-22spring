...
def make_diff(data_train, data_test):
    ...
    diff_train = data_train.iloc[lambda x: x.index % 12 == 0]
    for i in range(2):
        diff1_add = data_train.iloc[lambda x: x.index % 12 == (i*4+3)]
        diff_train = np.hstack((diff_train, diff1_add))

    diff_test = data_test.iloc[lambda x: x.index % 12 == 0]
    for i in range(2):
        diff2_add = data_test.iloc[lambda x: x.index % 12 == (i*4+3)]
        diff_test = np.hstack((diff_test, diff2_add))

    # diff_train = imp.fit_transform(diff_train)
    # diff_train = scaler.fit_transform(diff_train)
    # diff_test = imp.transform(diff_test)
    # diff_test = scaler.transform(diff_test)
...
x_train_diff, x_test_diff = make_diff(df_train_features, df_test_features)
x_train = np.append(x_train, x_train_diff, axis=1)
x_test = np.append(x_test, x_test_diff, axis=1)
...
#---------------------- Sub Task 3 ----------------------#
# parameters2  = {'alpha':[4.5, 5, 5.5,6,6.5]}
parameters2 = np.array([10.0, 9.5, 9.5, 9.5])
...
#---------------------- Output ----------------------#
y_pred.to_csv('prediction11.zip', index=False, float_format='%.3f', compression='zip')
y_pred.to_csv('prediction11.csv', index = False, float_format='%.3f')